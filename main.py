...
async def worker(token, index):
    ...
    try:
        ...
        # Verifying token with email
        #email = input("Email >")
        #password = input("Password >")
        #print(await client.actions.misc.verify.email.add(email, password))

        #verifyToken = input("verifyToken> ") # The verify token looks like eyJpZCI6MTQ...
        #verifyUrl = input("verifyUrl> ")
        
        #print(await client.actions.misc.verify.email.verify(verifyToken, verifyUrl))
